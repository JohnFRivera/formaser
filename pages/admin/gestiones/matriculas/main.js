$.ajax({
    url: "https://formarjkh.000webhostapp.com/formaser/back/modulos/gestionar_matriculados.php",
    dataType: "json",
    ...
    success: function (data) {
      var data_bd = [];
      for (var i in data) {
          data_bd.push(data[i].cedula);
          data_bd.push(data[i].nombre);
          data_bd.push(data[i].ficha);
          data_bd.push(data[i].programa);
          data_bd.push(data[i].estado);
      }
      let datos = document.getElementById("datos");
      for (var i in data_bd )
          {
              let fila = `
              <tr>
              <td> ${data_bd[i].cedula}</td>
              <td> ${data_bd[i].nombre}</td>
              <td> ${data_bd[i].ficha}</td>
              <td> ${data_bd[i].programa}</td>
              <td> ${data_bd[i].estado}</td>
              </tr>`;
